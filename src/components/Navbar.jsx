...
const Navbar = () => {
  const [nav, setNav] = useState(false) //nav=false(navbar close) nav=true(navbar open)

  const toggleNav = () => { //If nav is true(open) close(false) it , and vice versa
    setNav(!nav)
  }

  const closeNav = () => { //close the navBar
    setNav(false)
  }
  ...
  return (
    <div className='fixed top-0 left-0 w-full bg-opacity-70 backdrop-blur-md z-50'>
      <div className='max-w[1300px] mx-auto flex justify-between text-gray-100 text-xl items-center px-12 h-20'>
            ...
            {/* For other screen except mobile */}
